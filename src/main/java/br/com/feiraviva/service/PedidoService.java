...
public class PedidoService {
    ...
    public PedidoResponseDTO finalizar(Long clienteId, PedidoRequestDTO dto) {
        ...
        var estrategia = calculadoraFrete.normalizar(carrinho.getEstrategiaFrete());
        var frete = calculadoraFrete.calcular(estrategia, subtotal);
        var desconto = carrinho.getCodigoCupom() == null
                ? BigDecimal.ZERO
                : cupomFactory.criar(carrinho.getCodigoCupom()).calcularDesconto(subtotal);
        pedido.setSubtotal(subtotal);
        pedido.setFrete(frete);
        pedido.setTotal(subtotal.add(frete).subtract(desconto));
